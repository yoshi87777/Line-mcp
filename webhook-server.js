...
async function runConversationAgent(userMessage, history, systemPrompt, tools, ariaUserId) {
      ...
      try {
        ...
        while (response.functionCalls && response.functionCalls.length > 0) {
          ...
          for (const call of response.functionCalls) {
            ...
            const result = await executeTool(call.name, call.args, ariaUserId);
...
async function handleDM(replyToken, lineUserId, userMessage) {
  ...
  if (lineUserId !== YOSHIKI_USER_ID) {
    ...
    const reply = await runConversationAgent(userMessage, history, SECRETARY_SYSTEM_PROMPT, null, null);
